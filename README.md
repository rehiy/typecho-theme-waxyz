...
新建独立页面，选择模板为`友情链接`，可在编辑器内按如下 Markdown 表格填写；也可以让访客按固定评论格式提交友链申请，评论通过审核后会自动展示到友链列表，且不再重复显示在评论区。不填写头像、头像不是有效 URL 或加载失败时，会自动使用默认头像。Markdown 表格会读取前 5 列，多余列会自动忽略。
...
评论提交格式如下，必填站点名称和站点地址，`头像`可选，`简介`和`寄语`都可作为友链描述；字段冒号可使用中文/英文冒号或等号，前后空格不影响解析。字段名按关键词识别，例如名称类包含`名称/名字/站名`，地址类包含`网址/地址/链接/站点`，头像类包含`头像/图标/图片/logo`：

```text
名称：昏黎站
网址：https://hunli.100w.top/
头像:https://pic1.imgdb.cn/item/69c158bcac52b0dbce273886.jpg
寄语：年轻多做点cool的事！
```
